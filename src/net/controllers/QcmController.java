...
public class QcmController implements SelectionListener {
	...
	public void AddQcm(){
		...
		if(checkQuestGroupe==true && (nbTrueAnswer==1 || nbTrueAnswer==2)){
			...
			if(AppController.getSession_Id()==null){
				//Insertion d'une question appartenant � un questionnaire
				laQuestion.setQuestionnaire_id(insertQuestionnaire.getId());
			}else{
				//Sinon on prend l'id du questionnaire pr�c�dent
			...
			if(AppController.getSession_Id()==null){
				//Si tout est correct alors on associe les pr�c�dents ajout � l'utilisateur (s'il la relation n'existe pas encore)
	 ...
	 * @return L'id de l'�l�ment modifi�
	 */
	public Question updateQuestion(){
		page=Integer.valueOf(vAccueil.getLblCurrentQuestion().getText());
		//L'index commencant � 0 et non � 1 on d�cremante
		page--;
		//R�cup�rer la question � mettre � jour
		lesQuestions=Http.getQuestionByQuestionnaire(leQuestionnaire.getId());
		Integer nbRound=0;
	...
	/**
	 * Mettre � jour l'association entre groupe et questionnaire
	 * @param questionnaire
	 * @return
	 */
	public GroupeQuestionnaire updateGroupeQuestionnaire(Questionnaire questionnaire){
		...
		System.out.println("Ins�rtion : "+groupeQuestionnaire);
